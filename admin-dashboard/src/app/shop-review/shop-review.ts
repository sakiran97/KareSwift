...
declare const L: any; // Leaflet library loaded via index.html
...
export class ShopReviewComponent implements OnInit {
  ...
  // Photo Geotags (Simulated based on Shop Coordinates to test warning/pass status)
  photoLat = 0;
  photoLng = 0;
  distanceMeters = 0;
  ...
  selectShop(shop: any) {
    ...
    // Setup coordinates:
    const baseLat = shop.shopLatitude || 12.9716; // default fallback if null
    const baseLng = shop.shopLongitude || 77.5946;

    // Simulate photo geotag.
    // For demo/testing: every second shop gets a mismatch (>500m), others get a pass (<100m)
    const seed = shop.userId % 2 === 0;
    const offsetLat = seed ? 0.0055 : 0.0006; // 0.0055 lat difference is ~610m
    ...
    // Render the Leaflet map after DOM updates
    setTimeout(() => {
  ...
  initMap(lat: number, lng: number) {
    ...
    try {
      ...
      // Fit map bounds to contain both markers
